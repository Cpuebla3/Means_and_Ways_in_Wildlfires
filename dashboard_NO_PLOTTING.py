...
# Global Simulation Parameters Loading
# _______________________________________________________________

with open("Global_parameters.txt", "r") as file:
    lines = file.readlines()
    lines = [i.strip() for i in lines if '#' not in i]
    lines = [i.replace(' ', '') for i in lines if len(i)!=0]

input_parameters={j.partition('=')[0]:j.partition('=')[2] for j in lines}
...
tif_list=("cali_test_big_enhanced.tif", "marshall_enhanced.tif", "camp_fire_three_enhanced.tif")
scenario=tif_list[int(input_parameters['scenario'])]
final_image=input_parameters['final_image']
groundcrew_sector_mapping = [0]
second_groundcrew_sector_mapping = None

# MCTS and simulation configuration
decision_interval   = int(input_parameters['decision_interval'])   # minutes between replanning
mcts_iterations     = int(input_parameters['mcts_iterations']) 
...
iters_for_2_assets    = int(input_parameters['iters_for_2_assets']) 
iters_for_3_assets    = int(input_parameters['iters_for_3_assets']) 

# Simulation and fire parameters
elapsed_minutes     = int(input_parameters['elapsed_minutes'])
fire_sim_time       = int(input_parameters['fire_sim_time'])
overall_time_limit  = int(input_parameters['overall_time_limit'])
groundcrew_count    = int(input_parameters['groundcrew_count'])

# Airtanker counts (using one type for brevity)
c130j_count         = int(input_parameters['c130j_count'])
fireherc_count      = int(input_parameters['fireherc_count'])
scooper_count       = int(input_parameters['scooper_count'])
at802f_count        = int(input_parameters['at802f_count'])
dash8_count         = int(input_parameters['dash8_count'])

time_step           = int(input_parameters['time_step'])
operational_delay   = int(input_parameters['operational_delay'])
groundcrew_speed    = int(input_parameters['groundcrew_speed'])
wind_speed          = int(input_parameters['wind_speed'])
wind_dir            = int(input_parameters['wind_dir'])
wind_schedule_flag  = True if input_parameters['wind_schedule']=="True" else False

if wind_schedule_flag:
...
def build_model() -> WildfireModel:
    ...
    model = WildfireModel(
        ...
        enable_plotting      = True,  # disable all plotting/panel features
